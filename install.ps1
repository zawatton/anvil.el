...
$Repo = 'https://github.com/zawatton/anvil.el.git'
...
Invoke-Step "ensure $InitEl loads anvil-init" {
    if (-not (Test-Path $InitEl)) {
        ";;; init.el --- generated by anvil installer`r`n$hookLine`r`n" |
            Set-Content -Path $InitEl -Encoding UTF8
        Log "  created $InitEl"
    } elseif (-not (Select-String -Path $InitEl -SimpleMatch 'anvil-init.el' -Quiet)) {
        Add-Content -Path $InitEl -Value "`r`n;; added by anvil installer`r`n$hookLine"
        Log "  appended anvil-init hook to $InitEl"
    } else {
        Log "  $InitEl already loads anvil-init (ok)"
    }
}
