...
* STATUS
~Phase 1+2+3+4a+4c SHIPPED 2026-04-16~ — lane split + ~:kind~ + classifier
+ batch warmup + latency metrics + ~server-eval-at~ 既定 transport (per-
call emacsclient spawn を排除、~emacsclient~ path は fallback として
~anvil-worker-connection-method~ で切替可) + 52 worker ERT。
4b (worker 側 persistent listener with keepalive) は 4c baseline 再測後に判断。
