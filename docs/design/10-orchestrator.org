...
#+date: 2026-04-18
#+author: zawatton
#+property: status PHASE-7C-SHIPPED
...
- *Primitive (GENERIC 化可能)* — multi-provider dispatch / collect / consensus /
  judge / retry (jitter+cap, Retry-After, per-provider backoff) / DAG resume /
  live streaming / preamble / cross-session stats。provider adapter 層と
  MCP tools は multi-LLM 環境全般に有用。
- *Convention (運用者固有)* — 「claude=設計、codex=実装、judge=claude 既定」の
  役割分担慣習、journal MEMO 自動追記、Co-Authored-By 強制、denki-kanri SaaS
  ドメイン連携 (月次/年次点検報告書)、codex+ollama を cost-0 前提とした dispatch
  policy。公開時は =anvil-orchestrator-roles-default.el= 等に分離し、例示は neutral な
  architect/implementer に差し替え。
