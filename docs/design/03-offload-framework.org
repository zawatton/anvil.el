...
- Phase 3a (budget-exceed: hard-kill + slot respawn + `:resumable t' partial):
  ~SHIPPED~ (2026-04-17)
  - 新公開 API: ~anvil-future-kill FUTURE~ / ~anvil-future-elapsed FUTURE~
    / status ~'killed~
  - ~anvil-server--offload-apply~ は timeout 時に ~anvil-future-kill~ で
    subprocess slot ごと hard-kill → sentinel が slot nil 化 → 次 dispatch
    で respawn (以前は cancel のみで subprocess が生き残り slot が wedge)
  - tool plist 新 keyword: ~:resumable t~ — budget-exceeded 時に error では
    なく `(:status 'partial :value nil :cursor nil :consumed-sec N
    :reason 'budget-exceeded)` を serialize して返す
  - tests: kill-respawn PID 不一致確認 / resumable partial plist 返却確認 /
    既存 timeout test は文言 update ("budget exceeded")
- Phase 3b (checkpoint protocol + cursor + :value 保存): ~SHIPPED~ (2026-04-17)
  - 新公開 API: ~anvil-preempt-checkpoint VALUE &optional CURSOR~ (handler-side)
    / ~anvil-future-checkpoint FUTURE~ (daemon-side accessor)
  - Protocol 拡張: subprocess が intermediate ~(:id N :checkpoint
    (:value V :cursor C))~ を送信、main daemon の filter は非 settling で
    future の ~checkpoint~ slot を更新 (last-write-wins)
  - REPL init file が ~anvil-preempt-checkpoint~ を defun し、eval 中は
    ~anvil-offload--repl-current-id~ を dynamic-bind で current request-id に
    束縛 (tag 用)
  - ~anvil-server--offload-apply~: budget-exceed + ~:resumable t~ の時、
    kill 前に ~anvil-future-checkpoint~ を snapshot し、partial plist の
    ~:value~ / ~:cursor~ に畳み込む。checkpoint 未送信なら両方 nil (Phase 3a 互換)
  - daemon 側 ~anvil-preempt-checkpoint~ は no-op (handler が offload 外で
    呼ばれても安全)
  - tests: unit 2 件 (updates-future / last-write-wins) + E2E 1 件
    (resumable-folds-in-checkpoint)、total 247/249 (2 件は pre-existing Windows
    batch supersession flake)
- Phase 4: ~DRAFT~ — レビュー前。
- Phase 5 (heavy tool retrofit): ~SHIPPED~ 3rd wave (2026-04-17)
  ...
  - 3rd wave: ~org-index-rebuild~ retrofit — Phase 3b checkpoint protocol
    を最初に利用する production tool。新 defcustom
    ~anvil-org-index-rebuild-checkpoint-every~ (既定 25) で N 件毎に
    commit + `anvil-preempt-checkpoint` 発火。preempt された rebuild は
    partial plist の ~:cursor~ に `(:files-processed N :last-path P)` を
    載せて返し、already-committed chunk は DB に persist するので
    follow-up call で idempotent に再開可能。tool handler は
    ~anvil-org-index-enable~ を呼ばずに直接 DB open することで subprocess
    内での MCP tool 再登録を回避
  - tests: inherit-load-path がサイズ増加 / byte-compile 経由 MCP E2E /
    rebuild chunk毎 checkpoint / checkpoint-every=0 で no-op /
    rebuild offload E2E 全 250/252 green (2 件は pre-existing flake)
