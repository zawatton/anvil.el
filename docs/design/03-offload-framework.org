...
- Phase 3b (checkpoint protocol + cursor + :value 保存): ~DRAFT~ —
  handler 側で ~anvil-preempt-checkpoint VALUE CURSOR~ 呼び出し
  → subprocess が partial-reply 送信 → anvil-offload が最新 checkpoint を
  保持 → budget exceed 時に ~:value~ / ~:cursor~ 付きで返す
- Phase 4: ~DRAFT~ — レビュー前。
