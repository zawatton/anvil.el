...
- *Primitive (GENERIC 化可能)* — disk-as-truth policy、=file-*= tool の :warnings
  機構、buffer / disk divergence 検出 helper、=buffer-read= / =buffer-save= /
  =buffer-list-modified= MCP tools。大きい org / code base を扱う全環境で有効。
- *Convention (運用者固有)* — init.org の意図的 divergence 事故が設計動機の中核
  であり、=visited-modtime=0= / undo 無効 / tangle 途中 buffer を想定した guard
  が一部含まれる。公開時は general example (.emacs / config.org) に書き換え、
  本件事故の詳細は feedback memory へ退避。
