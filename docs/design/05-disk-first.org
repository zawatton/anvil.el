...
* STATUS
~Phase 1+2(tight) SHIPPED~ (2026-04-16) — Phase 1 helpers に加え、
~anvil-file-read~ / ~anvil-file-replace-string~ の返り値に ~:warnings~ を組込。
他 ~file-*~ tool (insert/delete/append/batch/replace-regexp) への展開と、
~buffer-*~ 系 MCP tool (Phase 3) は未着手。
