...
#   bash install.sh [--skip-emacs] [--prefix DIR] [--branch REF]
...
if [ "$DRY_RUN" -eq 0 ]; then
  if [ ! -f "$INIT_EL" ]; then
    printf ';;; init.el --- generated by anvil installer\n%s\n' "$HOOK_LINE" >"$INIT_EL"
    log "  created $INIT_EL"
  elif ! grep -Fq 'anvil-init.el' "$INIT_EL"; then
    printf '\n;; added by anvil installer\n%s\n' "$HOOK_LINE" >>"$INIT_EL"
    log "  appended anvil-init hook to $INIT_EL"
  else
    log "  $INIT_EL already loads anvil-init (ok)"
  fi
else
