#+title: anvil.el — Emacs as an MCP workbench for AI agents
#+author: zawatton
...
anvil.el is an *MCP server* that exposes a running Emacs (or a small
Rust runtime called NeLisp) as 40+ tools an AI agent can call out of the box.  Optional modules (memory / orchestrator / session / http / …) each add their own tools when you enable them.
...
v1.0 (2026-04) ships the first stable release, plus a Rust runtime
that lets you run anvil *without an Emacs install at all*.
