...
| =anvil-tools-by-intent= + =anvil-manifest= (v0.4.0) | Intent-based tool discovery plus =ANVIL_PROFILE= filter.  Tools tagged with =:intent / :layer / :stability= are discoverable via runtime query; =agent= / =edit= profiles reduce =tools/list= by ~5-10× for orchestrator child sessions (~17k → ~3k tokens of manifest) |
