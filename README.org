...
- *Bundled-Emacs tarball* — NeLisp Stage D v2.0 ships a stripped
  Emacs inside the archive; =bin/anvil mcp serve= works on hosts
  that never ran =apt install emacs=.
- *Rust binary, zero Emacs spawn* — =bin/anvil mcp serve --no-emacs=
  dispatches to the =anvil-runtime= Rust binary, implementing the
  Elisp reader, evaluator, MCP stdio JSON-RPC server, and an
  =anvil-host-*= tool registry.  An MCP client can call anvil tools
  through this path with no Emacs process spawn (NeLisp Doc 44
  LOCKED 2026-04-27).
