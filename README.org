...
but you can point one or more machines at the same SQLite file
(commonly stored inside a notes git repo) without symlinks:
...
module is using.  The DB is binary, so commit / pull it through
git as-is (run a =PRAGMA wal_checkpoint(FULL)= before commit so
the WAL writes flush into the main file).
