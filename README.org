...
Phase 1a ships one provider: =claude= (hard dependency on the
=claude= CLI on =exec-path=).  aider / gemini / ollama adapters land
in Phase 2+.  Concurrency caps (=anvil-orchestrator-concurrency=,
