...
v1.3.0 is focused on making Anvil useful beyond narrow file edits:
...
| =anvil-fusion= / long-run (v1.3.0) | Compare multiple agent outputs, keep compact panel summaries, store long-run results, and expose MCP surfaces for review/evaluation workflows |
